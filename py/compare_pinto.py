...
def flastKNN(outDir, projectBasePath, projectName, kf, dim, eps, k, sigma, params):
    ...
    dataLabelsList = np.array(
        [1]*len(dataPointsFlaky) + [0]*len(dataPointsNonFlaky))
    v1 = time.perf_counter()
    vecTime = v1 - v0

    # storage
    kNN = (dataPointsList, dataLabelsList)
    pickleDumpKNN = os.path.join(
        outDir, f"flast-k{k}-sigma{sigma}.pickle")
    ...
    for (trnIdx, tstIdx) in kf.split(dataPointsList, dataLabelsList):
        ...
        predictionTime = (vecTime / len(dataPoints)) + \
...
if __name__ == "__main__":
    ...
    # PINTO
    print("Pinto-KNN")
    (avgP, avgR, storage, avgTPrep, avgTPred) = pintoKNN(
        outDir, projectBasePath, projectName, kf)
    with open(os.path.join(outDir, outFile), "a") as fo:
        fo.write(f"Pinto-KNN,{avgP},{avgR},{storage},{avgTPrep},{avgTPred}\n")
    ...
    for k in [3, 7]:
        for sigma in [0.5, 0.95]:
            print("FLAST", k, sigma)
            (avgP, avgR, storage, avgTPrep, avgTPred) = flastKNN(
                outDir, projectBasePath, projectName, kf, dim, eps, k, sigma, params)
            with open(os.path.join(outDir, outFile), "a") as fo:
                fo.write(
                    f"FLAST-k{k}-sigma{sigma},{avgP},{avgR},{storage},{avgTPrep},{avgTPred}\n")
