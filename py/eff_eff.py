    ...
    for k in [3, 7]:
        for sigma in [0.5, 0.95]:
            for projectName in projectList:
                ...
                (flakyTrain, nonFlakyTrain, flakyTest, nonFlakyTest, avgP, avgR, storage, avgTPrep, avgTPred) = flastKNN(
                    outDir, projectBasePath, projectName, kf, dim, eps, k, sigma, params)
                with open(os.path.join(outDir, outFile), "a") as fo:
                    fo.write(
                        f"{projectName},{flakyTrain},{nonFlakyTrain},{flakyTest},{nonFlakyTest},{k},{sigma},{avgP},{avgR},{storage},{avgTPrep},{avgTPred}\n")
